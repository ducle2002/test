...
{
    public interface IAmenitiesItemAppService : IApplicationService
    {
        Task<object> GetAll(GetAllAmenitiesItemInput input);
        Task<object> GetById(long id);
        Task<object> Create([FromBody] AmenitiesItemDto input);
        Task<object> Update([FromBody] AmenitiesItemDto input);
        Task<object> Delete(long id);
        Task<object> DeleteMany([FromBody] List<long> ids);
    }

    public class AmenitiesItemAppService : YootekAppServiceBase, IAmenitiesItemAppService
    {
        private readonly IAbpSession _abpSession;
        private readonly BaseHttpClient _httpClient;
        private string baseUrl = "api/v1/AmenitiesItem/";

        public AmenitiesItemAppService(IAbpSession abpSession, IConfiguration configuration)
        {
        ...
        public async Task<object> Create([FromBody]AmenitiesItemDto input)
        {
            try
            {
        ...
        public async Task<object> Update([FromBody]AmenitiesItemDto input)
        {
            try
            {
