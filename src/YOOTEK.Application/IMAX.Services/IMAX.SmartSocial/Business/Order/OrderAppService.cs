...
using PRecipientAddress = IMAX.Application.Protos.Business.Orders.PRecipientAddress;

namespace IMAX.Services.SmartSocial.Orders
{
    ...
    public class OrderAppService : IMAXAppServiceBase, IOrderAppService
    {
        ...
        public async Task<DataResult> CreateOrderAsync(CreateOrderByUserInputDto input)
        {
            try
            {
                ...
                if (input.PaymentMethod != PAYMENT_METHOD.COD)
                {
                    CreatePaymentDto requestPayment = new()
                    {
                        TenantId = input.TenantId,
                        TransactionId = result.Data,
                        Method = (EPaymentMethod)input.PaymentMethod,
                        Type = EPaymentType.SHOPPING,
                        Description = string.Empty,
                        Amount = (long)input.TotalPrice,
                        Properties = string.Empty,
                    };
                    string paymentId = await _httpPaymentService.CreatePaymentAsync(requestPayment);
                    if (paymentId is null)
                    {
                        throw new UserFriendlyException("Payment fail");
                    }
                    ...
                    return DataResult.ResultSuccess(paymentId, "Create order success");
                }
                else
                {
                    await PushNotificationCreateOrder(new()
        ...
        private async Task RatingOrder(POrder order)
        {
            if (order.State == (int)STATE_ORDER.USER_COMPLETED)
            {
                await _orderProtoClient.UpdateStateOrderAsync(
                    new UpdateStateOrderRequest()
                    {
                        Id = order.Id,
                        CurrentState = (int)STATE_ORDER.USER_COMPLETED,
                        UpdateState = (int)STATE_ORDER.USER_RATING,
                    }, MetadataGrpc.MetaDataHeader(AbpSession));
            }
            else throw new UserFriendlyException("Cannot rating order");
        }
