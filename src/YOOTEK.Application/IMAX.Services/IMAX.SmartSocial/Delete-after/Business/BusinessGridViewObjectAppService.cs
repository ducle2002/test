...
namespace IMAX.IMAX.Services.IMAX.DichVu.Business
{
    ...
    public class BusinessGridViewObjectAppService : IMAXAppServiceBase, IBusinessGridViewObjectAppService
    {
                ...
                var query = (from obj in _objectPartnerRepos.GetAll()
                             ...
                             select new ObjectDto()
                             {
                                 ...
                                 Rates = (from rate in _rateRepos.GetAll()
                                          ...
                                          select rate)
                                          .ToList(),
                                 CountRate = obj.IsDataStatic.Value ? obj.CountRate.Value : (from rate in _rateRepos.GetAll()
                                                                                             where rate.ObjectId == obj.Id
                                                                                             select rate).AsQueryable().Count(),
                                 Rate = obj.IsDataStatic.Value ? (double)Math.Round(obj.RatePoint.Value, 1) : (double)Math.Round((double)Queryable.Average(from rate in _rateRepos.GetAll()
