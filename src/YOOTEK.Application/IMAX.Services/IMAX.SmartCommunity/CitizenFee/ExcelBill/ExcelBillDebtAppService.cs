...
namespace IMAX.Services.SmartCommunity.ExcelBill
{
    ...
    public class ExcelBillDebtAppService : IMAXAppServiceBase, IExcelBillDebtAppService
    {
            ...
            ExcelPackage package, string filePath, FileStream stream)
        {
            using (CurrentUnitOfWork.SetTenantId(AbpSession.TenantId))
            {
                try
                {
                   ...
                   // await InsertBillToDatabase(excelBillDebtDtos);
            ...
            for (int row = 2; row <= rowCount; row++)
            {
                ...
                for (int i = 0; i < userBillTypes.Count; i++)
                {
                    ...
                    UserBill userBillTemp = new()
                    {
                        ...
                        paymentMethod = (UserBillPaymentMethod?)paymentMethod,
                        BillConfigId = 0,
                    };
        ...
        //private async Task<Unit> InsertBillToDatabase(List<ExcelBillDebtDto> excelBillDebtDtos)
        //{
        //    if (excelBillDebtDtos == null || excelBillDebtDtos.Count == 0) return Unit.Value;
        //    foreach (ExcelBillDebtDto excelBillDebtDto in excelBillDebtDtos)
        //    {
        //        List<long> userBillIds = new();
        //        long paymentId = 0;
        //        List<UserBill> userBills = excelBillDebtDto.UserBills;
        //        if (userBills.Count == 0) return Unit.Value;
        //        foreach (UserBill userBill in userBills)
        //        {
        //            long id = await _userBillRepository.InsertAndGetIdAsync(userBill);
        //            userBill.Id = id;
        //            userBill.Code = $"HD{id}{userBill.Period:MMyyyy}";
        //            await _userBillRepository.UpdateAsync(userBill);
        //            userBillIds.Add(id);
        //        }
        //        List<UserBillPayment>? userBillPayments = excelBillDebtDto.UserBillPayments;
        //        if (userBillPayments != null && userBillPayments.Count > 0)
        //        {
        //            UserBillPayment userBillPayment = userBillPayments[0];
        //            userBillPayment.UserBillIds = string.Join(",", userBillIds.OrderBy(x => x));
        //            paymentId = await _userBillPaymentRepository.InsertAndGetIdAsync(userBillPayment);
        //            userBillPayment.Id = paymentId;
        //            userBillPayment.PaymentCode = $"PM-{paymentId}-{GetUniqueKey(6)}";
        //            _userBillPaymentRepository.Update(userBillPayment);
        //        }
        //        List<BillDebt>? billDebts = excelBillDebtDto.BillDebts;
        //        if (billDebts != null && billDebts.Count > 0)
        //        {
        //            BillDebt billDebt = billDebts[0];
        //            billDebt.UserBillIds = string.Join(",", userBillIds.OrderBy(x => x));
        //            billDebt.BillPaymentId = paymentId > 0 ? paymentId : 0;
        //            await _billDebtRepository.InsertAsync(billDebt);
        //        }
        //    }
