...
using IMAX.IMAX.Services.IMAX.SmartCommunity.CitizenFee;
...
namespace IMAX.Services
{
    ...
    public class BillEmailUtil : IMAXAppServiceBase, IBillEmailUtil
    {
        ...
        public async Task<StringBuilder> GetTemplateByApartment(string apartmentCode, DateTime period)
        {
            ...
            else if (AbpSession.TenantId == 87)
            {
        ...
        {
            await SendEmailToTenantAsync(apartmentCode, tim, tenantId);
        }

        private async Task SendEmailToTenantAsync(string apartmentCode, DateTime? tim, int? tenantId)
        {
            using (CurrentUnitOfWork.SetTenantId(tenantId))
            {
                ...
                var emailTemplate = GetTemplateByApartment(apartmentCode, time);
        ...
        public async Task<StringBuilder> CreateTemplateVina22(string apartmentCode, DateTime period, int? tenantId)
        {
            using (CurrentUnitOfWork.SetTenantId(tenantId))
            {
                ...
                double managementMoneyDebt = CalculateDebt(userBills, BillType.Manager, preMonthPeriod);
                double parkingMoneyDebt = CalculateDebt(userBills, BillType.Parking, preMonthPeriod);
                double waterMoneyDebt = CalculateDebt(userBills, BillType.Water, preMonthPeriod);
        ...
        public async Task<StringBuilder> CreateTemplateGeneral(string apartmentCode, DateTime period, int? tenantId)
        {
            using (CurrentUnitOfWork.SetTenantId(tenantId))
            {
                ...
                List<BillConfig> billConfigs = _billConfigRepository.GetAllList();
                UserBill parkingBill = userBills.FirstOrDefault(x => x.BillType == BillType.Parking) ?? new UserBill();
                ...
                List<UserBill> otherBills = userBills.Where(x => x.BillType == BillType.Orther).ToList();
                ...
                double parkingMoneyDebt = CalculateDebt(userBills, BillType.Parking, preMonthPeriod);
                double waterMoneyDebt = CalculateDebt(userBills, BillType.Water, preMonthPeriod);
                double electricMoneyDebt = CalculateDebt(userBills, BillType.Electric, preMonthPeriod);
                double lightMoneyDebt = CalculateDebt(userBills, BillType.Lighting, preMonthPeriod);
                double otherMoneyDebt = CalculateDebt(userBills, BillType.Orther, preMonthPeriod);
                ...
                double otherMoneyPrePayment = CalculatePrePayment(userBills, BillType.Orther);
                ...
                if (billWaterConfigProperties.Prices != null && billWaterConfigProperties.Prices.Any())
                {
                    ...
                    foreach (var (unitPrice, index) in listUnitPrices.Select((unitPrice, index) => (unitPrice, index)))
                    {
                        ...
                        StringBuilder waterRowTemplate = new(GetListWaterConsumption(tenantId));
                        ...
                        if (index == 0)
                        {
                            waterConsumptionContent = $"- {unitPrice.To}m3 đầu tiên";
                        }
                        else if (unitPrice.To.HasValue)
                        {
                            waterConsumptionContent = $"- Từ {unitPrice.From}m3 đến {unitPrice.To}m3";
                        }
                        else
                        {
                            waterConsumptionContent = $"- Từ {unitPrice.From}m3 trở đi";
                        }
                ...
                if (billElectricConfigProperties.Prices != null && billElectricConfigProperties.Prices.Any())
                {
                    ...
                    foreach (var (unitPrice, index) in listUnitPrices.Select((unitPrice, index) => (unitPrice, index)))
                    {
                        ...
                        StringBuilder electricRowTemplate = new(GetListElectricConsumption(tenantId));
                        ...
                        if (index == 0)
                        {
                            electricConsumptionContent = $"- {unitPrice.To} số đầu tiên";
                        }
                        else if (unitPrice.To.HasValue)
                        {
                            electricConsumptionContent = $"- Từ số {unitPrice.From} đến số {unitPrice.To}";
                        }
                        else
                        {
                            electricConsumptionContent = $"- Từ số {unitPrice.From} trở đi";
                        }
                ...
                if (listVehicles.Count > 0)
                {
                    foreach (var (vehicle, index) in listVehicles.Select((vehicle, index) => (vehicle, index)))
                    {
                        StringBuilder vehicleRowTemplate = new(GetListVehicle(tenantId));
                        string vehicleName = GetVehicleName(vehicle);
                        string vehicleCode = vehicle?.vehicleCode ?? "";
                        decimal vehicleCost = vehicle?.cost ?? 0;
               ...
               // StringBuilder emailTemplate = new(_templateBillAppService.GetContentOfTemplateBill(new GetTemplateOfTenantInput() { TenantId = tenantId, Type = ETemplateBillType.BILL }));
                ...
                emailTemplate
                    ...
                    // parking
                    .Replace("{UNIT_PRICE_PARKING}", "")
                    .Replace("{UNIT_PRICE_CAR}", FormatCost(billParkingConfigProperties?.Prices[0]?.Value))
                    .Replace("{UNIT_PRICE_MOTOR}", FormatCost(billParkingConfigProperties?.Prices[1]?.Value))
                    .Replace("{UNIT_PRICE_BIKE}", FormatCost(billParkingConfigProperties?.Prices[2]?.Value))
                    .Replace("{UNIT_PRICE_OTHER}", FormatCost(billParkingConfigProperties?.Prices[3]?.Value))
                    .Replace("{CAR_NUMBER}", GetStringValue(parkingBill.CarNumber))
                    .Replace("{MOTOR_NUMBER}", GetStringValue(parkingBill.MotorbikeNumber))
                    .Replace("{BIKE_NUMBER}", GetStringValue(parkingBill.BicycleNumber))
                    .Replace("{OTHER_NUMBER}", GetStringValue(parkingBill.OtherVehicleNumber))
                    .Replace("{COST_CAR_UNPAID}", FormatCost(parkingBill.CarNumber * billParkingConfigProperties?.Prices[0]?.Value))
                    .Replace("{COST_MOTOR_UNPAID}", FormatCost(parkingBill.MotorbikeNumber * billParkingConfigProperties?.Prices[1]?.Value))
                    .Replace("{COST_BIKE_UNPAID}", FormatCost(parkingBill.BicycleNumber * billParkingConfigProperties?.Prices[2]?.Value))
                    .Replace("{COST_OTHER_UNPAID}", FormatCost(parkingBill.OtherVehicleNumber * billParkingConfigProperties?.Prices[3]?.Value))
                    .Replace("{COST_PARKING_UNPAID}", FormatCost(parkingMoney))
                    .Replace("{COST_WATER_PRE_PAYMENT}", FormatCost(parkingMoneyPrePayment))
                    .Replace("{COST_PARKING_DEBT}", FormatCost(parkingMoneyDebt))
                    .Replace("{COST_PARKING}", FormatCost(parkingMoney))
                    ...
                    .Replace("{LIST_OTHER_BILL}", $"{listRowVehicles}")
        ...
        public async Task<StringBuilder> CreateTemplateHTS(string apartmentCode, DateTime time, int? tenantId)
        {
            using (CurrentUnitOfWork.SetTenantId(tenantId))
            {
                ...
                var bills = _userBillRepository.GetAll().Where(x => x.ApartmentCode == apartmentCode && x.Period.Value.Month == time.Month && x.Period.Value.Year == time.Year && x.IsDraft != true).ToList();
                var citizenTemp = _citizenTempRepository.FirstOrDefault(x => x.ApartmentCode == apartmentCode && x.RelationShip == RELATIONSHIP.Contractor && x.IsStayed == true);
                var taxCode = "";
                if (citizenTemp == null)
                {
                            ...
                            switch (vehicle.vehicleType)
                            {
                                case VehicleType.Car:
                                    try
                                    {
                                        ...
                                        var name = $"Phí gửi ô tô số {vehicle.level ?? 0}";
                                ...
                                case VehicleType.Motorbike:
                                    try
                                    {
                                        ...
                                        var name = $"Phí gửi xe máy số {vehicle.level ?? 0}";
                                ...
                                case VehicleType.Bicycle:
                                    try
                                    {
                                        ...
                                        var name = $"Phí gửi xe đạp số {vehicle.level ?? 0}";
                ...
                //hóa đơn khác
                var t_money = 0;
                var otherBills = bills.Where(x => x.BillType == BillType.Orther).ToList();
        ...
        {
            if (
                (await SettingManager.GetSettingValueForTenantAsync(EmailSettingNames.DefaultFromAddress, AbpSession.TenantId.Value)).IsNullOrEmpty() ||
                (await SettingManager.GetSettingValueForTenantAsync(EmailSettingNames.Smtp.Host, AbpSession.TenantId.Value)).IsNullOrEmpty()
            )
            {
