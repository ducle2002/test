...
namespace IMAX.Services
{
    ...
    [AbpAuthorize]
    public class UserCitizenAppService : IMAXAppServiceBase, IUserCitizenAppService
    {
        ...
        public async Task<object> GetAllCitizenApartment(GetAllCitizenApartment input)
        {
            try
            {
                List<CitizenDto> listCitizenApartment = _citizenRepos.GetAll()
                    .Where(x => x.AccountId == AbpSession.UserId)
                    .WhereIf(input.State.HasValue, x => (int)x.State == input.State)
                    .Select(x => new CitizenDto()
                    {
                        Id = x.Id,
                        TenantId = x.TenantId,
                        FullName = x.FullName,
                        ...
                        IsStayed = x.IsStayed,
                        IsVoter = x.IsVoter,
                        MemberNum = x.MemberNum,
                        Nationality = x.Nationality,
                        OrganizationUnitId = x.OrganizationUnitId,
                        OtherPhones = x.OtherPhones,
                        PhoneNumber = x.PhoneNumber,
                        UrbanId = x.UrbanId,
                        RelationShip = x.RelationShip,
                        State = x.State,
                        Type = x.Type,
                        UrbanCode = x.UrbanCode,
                        BuildingName = _organizationUnitRepository.GetAll().Where(i => i.Code == x.BuildingCode)
                            .Select(x => x.DisplayName).FirstOrDefault(),
                        ApartmentName = _apartmentRepository.GetAll().Where(i => i.ApartmentCode == x.ApartmentCode)
        ...
        public async Task<object> UpdateCitizen(UpdateCitizenByUserInput input)
        {
            try
            {
                ...
                citizenUpdate.State = input.State;
